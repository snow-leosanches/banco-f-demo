...
const customerMemoryAttributes = [
  {
    aggregation: 'unique_list',
    description: 'Distinct benefit catalog IDs the customer opened in the last hour',
    events: [schemaEvent('benefit_viewed')],
    name: 'benefits_visited_last_1h',
    period: 'PT1H',
  ...
  {
    aggregation: 'unique_list',
    description: 'Distinct benefit merchants the customer opened in the last hour',
    events: [schemaEvent('benefit_viewed')],
    name: 'merchants_visited_last_1h',
    period: 'PT1H',
    property: eventProperty('benefit_viewed', '$.merchant'),
    type: 'string_list',
  },
...
export const customerIdAttributesGroup = {
  ...
  description:
    'Customer memory for the Banco F Signals POC: benefits and merchants visited in the last hour, plus 7-day ping/session volume for estimated average engaged session',
  is_published: true,
