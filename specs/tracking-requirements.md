...
- Stream attribute group `banco_falabella_customer_id_attributes`, key `customer_id`: `benefits_visited_last_1h`, `merchants_visited_last_1h` (`period: PT1H`), `page_pings_last_7d`, `sessions_last_7d` (`period: P7D`) — **published**. The 1-hour unique lists reset between demos on the same login. The panel derives estimated average engaged session as `(page_pings_last_7d × 10s) / sessions_last_7d`. The Asistente loads this group with `getGroupAttributes`.
